...
% doesn't help, we still can't use the minted env inside overlays such as \uncover, muse use \inputminted ... hmpf
%%\setminted{
%%    beameroverlays=true,
%%    }
...
%%    \uncover<4->{
%%        \vfill
%%        $x_j=x_1: \pdi{x_1}{x_1}=1, \pdi{x_2}{x_1}=0$
%%        {\small
%%        \begin{equation*}
...
\begin{frame}
    ...
    \uncover<2->{
        Augment each intermediate $\ve z(\cdot) = \ve a(\cdot), \ve b(\cdot), \ve c(\cdot)$ with
        \begin{equation*}
            \text{VJP: }\blue{\ve v^\top}\,\red{\pd{\ve z}{*}}
        \end{equation*}
    }
    \uncover<3->{
        {\small
